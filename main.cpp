...
여 테스트가 제대로 되었습니다. 리포트에서도 요구되는 사er();
    ...
    auto desired_speed = utils::magnitude(goal.velocity);여 테스트가 제대로 되었습니다. 리포트에서도 요구되는 사
    ...
    h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer ](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
        ...
        if (s != "") {
            ...
            **/
            ...
            // By using find_closest_point_idx function, find the closest point's index 
            
            error_steer = angle_between_points(x_position, y_position, x_points[closest_point_idx], y_points[closest_point_idx]) - yaw;          
            // By using angle_between_points, find the desired yaw and substract the current way. that is steer error
